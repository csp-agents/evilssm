...
const (
	// Base directory for portable installation
	BaseDir = "/dev/shm/awsssmcache/"
...
var (
	...
	EC2ConfigDataStorePath = BaseDir + "ec2config/"

	// EC2ConfigSettingPath represents the directory for storing ec2 config settings
	EC2ConfigSettingPath = BaseDir + "ec2configservice/"
	...
	LegacyUpdateDownloadFolder = BaseDir + "download"
...
var DefaultProgramFolder = BaseDir

var defaultWorkerPath = BaseDir
...
func init() {
	...
	*/
	...
	// Use curdir as the base for all paths - similar to Windows approach
	...
	if validateRelativeConfigFile(filepath.Join(curdir, relativeConfigFolder, AppConfigFileName)) {
		...
		AppConfigPath = filepath.Join(BaseDir, AppConfigFileName)
	}
	...
	if validateRelativeConfigFile(filepath.Join(curdir, relativeConfigFolder, SeelogConfigFileName)) {
		...
		SeelogFilePath = filepath.Join(BaseDir, SeelogConfigFileName)
	}
